...
{
    [SerializeField]
    public GameController gameController;   // 콤보 초기화를 위한 GameController

    public float waitTimeOnGround;   // 지면에 올라와서 내려가기까지 기다리는 시간 

    public float limitMinY;   // 내려갈 수 있는 최소 y 위치

    public float limitMaxY;   // 올라올 수 있는 최대 y 위치
    ...
    /// <summary>
    /// 두더지가 바닥에서 대기하는 상태로 최초의 바닥 위치로 두더지 위치 설정
    /// </summary>
    ...
    /// <summary>
    /// 두더지가 홀 밖으로 나와있는 상태 (maxYPosOnGround 위치까지 위로 이동)
    /// </summary>
